#########
# this visualization function is flawless and symbolic for our assignment2. 
# I got inspired by this function and think the author could probably select more 
#countries instead of only two in to build up the complexity of plot
##########


# I add one more country to the function and it works, I can also add the fourth one and so on 
visulization_for_3<- function(data, country1, country2, country3, indicator){
  # ex: country1 <- "China"
  # ex: country2 <- "Australia"
  # ex: country3 <- "Japan"
  # indicator <- "life_expect"
  # indicator <- as.character(indicator)
  if(dim(filter(data, country == country1))[1] == 0 | dim(filter(data, country == country2))[1] == 0 | dim(filter(data, country == country3))[1] == 0){
  ...
  if(indicator == "life"){
    ...
    plot_data <- rbind(data.frame(country = data1$country, year = data1$year, life_expect = data1$life_expect),
                       ...
                       data.frame(country = data3$country, year = data2$year, life_expect = data3$life_expect),
                       mean_year)
    ...
    plot_data <- rbind(data.frame(country = data1$country, year = data1$year, income_per_person = data1$income_per_person),
                       ...
                       data.frame(country = data3$country, year = data3$year, income_per_person = data3$income_per_person),
                       mean_income)
